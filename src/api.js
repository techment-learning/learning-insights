...
const KEY_TO_COLLECTION = {
  ...
  "ltp-self-learning": "self_learning",
};
...
// Last-resort recovery when no one can sign in to click "regenerate
// passkey" for you — uses a secret pin set directly in the Worker's own
// settings (see DEPLOY.md), not stored anywhere in the app's database.
export async function recoverAdminAccess(email, pin, newPasskey) {
  const data = await apiFetch("/api/auth/recover-admin", { method: "POST", body: JSON.stringify({ email, pin, newPasskey }) });
  setSession(data.token, data.user);
  return data;
}
