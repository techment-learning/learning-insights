...
export function subscribeToKey(key, onChange) {
  ...
  if (config) {
    // Only the initial load does a full SELECT *. Every subsequent change
    // is applied incrementally from the realtime payload itself (which
    // Postgres already sends over the open websocket, at no extra API
    // cost) — not by re-querying the whole table again. Re-fetching
    // everything on every change, for every connected browser, is what
    // was driving egress usage far past the free tier's daily allowance.
    let currentRows = [];
    const emit = () => onChange(currentRows.map(config.fromDb));
    ...
    const applyChange = (payload) => {
      ...
      if (payload.eventType === "INSERT") {
        currentRows = [...currentRows.filter((r) => r.id !== payload.new.id), payload.new];
      } else if (payload.eventType === "UPDATE") {
        currentRows = currentRows.map((r) => r.id === payload.new.id ? payload.new : r);
      } else if (payload.eventType === "DELETE") {
        currentRows = currentRows.filter((r) => r.id !== payload.old.id);
      }
      emit();
    };
    ...
    const channel = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: config.table }, applyChange)
      .subscribe();
    return () => {
      active = false;
