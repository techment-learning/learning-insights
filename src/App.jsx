...
import {
  ...
  ClipboardList, ArrowLeft, Filter, RotateCcw, Pencil, Trash2, UserMinus, Download, Copy, Key
} from "lucide-react";
import {
  PieChart, Pie, Cell, BarChart, Bar, XAxis, YAxis, Tooltip, Legend,
  ResponsiveContainer, CartesianGrid
} from "recharts";
import { subscribeToKey, writeKey, readKey, signUpWithPassword, signInWithPassword, signOutUser, onAuthChange, addUserWithPasskey, regeneratePasskey } from "./api";
    ...
    <Shell>
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: 560, padding: 40 }}>
        ...
        <p style={{ color: T.inkMuted, fontSize: 13, marginBottom: 24, textAlign: "center" }}>
          {mode === "signin" ? "Sign in with the email and passkey your admin gave you." : "First-time setup — only works if no admin exists yet."}
        </p>
        ...
        <form onSubmit={handleSubmit} style={{ width: "100%", maxWidth: 340 }}>
          ...
          <Field label={mode === "signin" ? "Passkey" : "Choose a password"}>
          ...
          <Btn type="submit" disabled={busy} style={{ width: "100%", justifyContent: "center", opacity: busy ? 0.7 : 1, marginTop: 4 }}>
            {busy ? "Please wait…" : mode === "signin" ? "Sign in" : "Set up workspace"}
          </Btn>
        </form>
        ...
        <div style={{ marginTop: 18, fontSize: 12.5, color: T.inkMuted, textAlign: "center" }}>
          {mode === "signin" ? (
            <>
              ...
              <button onClick={() => { setMode("signup"); setFormError(null); }} style={{ background: "none", border: "none", color: T.inkFaint, cursor: "pointer", fontFamily: T.sans, fontSize: 11, marginTop: 8, textDecoration: "underline" }}>Setting this workspace up for the first time?</button>
            </>
          ) : (
