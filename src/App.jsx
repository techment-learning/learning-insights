...
import { subscribeToKey, writeKey, readKey, signUpWithPassword, signInWithPassword, sendPasswordReset, updatePassword, signOutUser, onAuthChange, inviteUser, getPendingResetToken } from "./api";
...
// A reset/invite link looks like yourapp.github.io/?resetToken=XXXX —
// detecting it directly from the URL is simpler than Supabase's approach,
// since our reset endpoint verifies the token itself rather than relying
// on an already-established "recovery session."
function hasPendingResetLink() {
  return !!getPendingResetToken();
}
...
export default function App() {
  ...
  const [forcePasswordSetup, setForcePasswordSetup] = useState(() => hasPendingResetLink());
  ...
  async function handleSubmit(e) {
    ...
    try {
      if (mode === "signin") {
        ...
        // straight in, so there's nothing further to prompt here.
...
function AddUserModal({ onClose, onSave }) {
  ...
  async function handleAdd() {
    ...
    try {
      await inviteUser(user.email, user.name, user.role);
