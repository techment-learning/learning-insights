...
import {
  ...
  ClipboardList, ArrowLeft, Filter, RotateCcw, Pencil, Trash2, UserMinus, Download
} from "lucide-react";
import {
...
export default function App() {
  ...
  const [showCreatePlan, setShowCreatePlan] = useState(null); // { training, willLock }
  ...
  async function handleDeleteTraining(training) {
    ...
    const nextTrainings = trainings.filter((t) => t.id !== training.id);
    const nextProgress = progress.filter((p) => p.trainingId !== training.id);
    await persist("ltp-trainings", nextTrainings, setTrainings);
    await persist("ltp-progress", nextProgress, setProgress);
  ...
  async function handleCreatePlan(training, topics, willLock) {
    const next = trainings.map((t) =>
      t.id === training.id ? { ...t, lessonPlan: topics, planLockedByLearner: willLock } : t
    );
  ...
  async function handleSubmitPlan(training) {
    ...
    const next = trainings.map((t) => t.id === training.id ? { ...t, planLockedByLearner: true } : t);
    await persist("ltp-trainings", next, setTrainings);
  ...
  return (
    <Shell>
      ...
      <div style={{ padding: 26, maxWidth: 1100, margin: "0 auto", width: "100%", boxSizing: "border-box" }}>
        ...
        {!isAdmin && view === "reports" && (
          <LearnerReports trainings={myEnrolled} progress={progress} currentUser={currentUser} />
        )}
      </div>
        ...
        <UpdateProgressModal training={showUpdate} currentUser={currentUser} progress={progress}
          ...
          onSaveMany={async (newEntries, removeTopicIds, training) => {
            ...
            const filtered = progress.filter((p) =>
              !(p.trainingId === training.id && p.userId === currentUser.id && p.topicId && touchedTopicIds.has(p.topicId))
            );
      ...
      {showEditTraining && (
        <EditTrainingModal training={showEditTraining} onClose={() => setShowEditTraining(null)}
          onSave={async (updated) => {
            const next = trainings.map((t) => t.id === updated.id ? updated : t);
            await persist("ltp-trainings", next, setTrainings);
...
// ---------- Admin Dashboard ----------
function AdminDashboard({ trainings, users, progress, alerts, onAddTraining, onEnroll, onAddTopic, onEditTopic, onDeleteTopic, onUnenroll, onCreatePlan, onEditTraining, onDeleteTraining }) {
    ...
    <div>
      ...
      {trainings.map((t) => (
        <TrainingCard key={t.id} training={t} users={users} progress={progress} onEnroll={() => onEnroll(t)} onAddTopic={() => onAddTopic(t)}
          ...
          onCreatePlan={() => onCreatePlan(t)} onEditTraining={onEditTraining} onDeleteTraining={onDeleteTraining} />
      ))}
    </div>
  );
}
...
function TrainingCard({ training, users, progress, onEnroll, onAddTopic, onEditTopic, onDeleteTopic, onUnenroll, onCreatePlan, onEditTraining, onDeleteTraining }) {
...
function TrainingsView({ trainings, users, progress, onAdd, onEnroll, onAddTopic, onEditTopic, onDeleteTopic, onUnenroll, onCreatePlan, onEditTraining, onDeleteTraining }) {
  if (trainings.length === 0) {
...
function ReportsView({ trainings, users, progress, onOpenReport }) {
  ...
  function downloadStatusTable() {
    downloadCSV("status-report", rows.map((r) => ({
      Learner: r.user, Training: r.training, "Planned %": r.plannedPct, "Actual %": r.actualPct, Status: r.status,
    })));
  }
  ...
  function downloadLearnerPlan() {
    if (!dlTraining || !dlLearnerId || !dlTraining.lessonPlan?.length) return;
    const learner = users.find((u) => u.id === dlLearnerId);
    downloadCSV(`${learner?.name || "learner"}-${dlTraining.title}-plan`, dlTraining.lessonPlan.map((l) => {
      ...
      <div style={{ background: T.surface, border: `1px solid ${T.line}`, borderRadius: 11, padding: 16, marginBottom: 20 }}>
        ...
        <div style={{ paddingTop: 12 }}>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
            ...
            <Btn variant="ghost" onClick={downloadLearnerPlan} style={{ opacity: dlLearnerId && dlTraining?.lessonPlan?.length ? 1 : 0.5 }}>
...
function LearnerReports({ trainings, progress, currentUser }) {
  const rows = useMemo(() =>
    trainings.map((t) => ({ training: t.title, trainingId: t.id, ...computeStatus(t, currentUser.id, progress) })),
    [trainings, progress, currentUser]
  );
  ...
  function downloadMyProgress() {
    downloadCSV("my-progress", rows.map((r) => ({
      Training: r.training, "Planned %": r.plannedPct, "Actual %": r.actualPct, Status: r.status,
    })));
  }
  ...
  return (
    <div>
      ...
      <div style={{ background: T.surface, border: `1px solid ${T.line}`, borderRadius: 11, overflow: "hidden" }}>
        <div style={{
          display: "grid", gridTemplateColumns: "1fr 90px 90px 110px", gap: 8, padding: "10px 16px",
          borderBottom: `1px solid ${T.line}`, fontFamily: T.mono, fontSize: 10.5, color: T.inkFaint, textTransform: "uppercase",
        }}>
          <span>Training</span><span>Planned</span><span>Actual</span><span>Status</span>
        </div>
        {rows.map((r, i) => (
          <div key={i} style={{
            display: "grid", gridTemplateColumns: "1fr 90px 90px 110px", gap: 8, padding: "11px 16px", alignItems: "center",
            borderBottom: i < rows.length - 1 ? `1px solid ${T.line}` : "none", fontSize: 13,
          }}>
...
function LearnerDashboard({ trainings, progress, currentUser, onUpdate, onAddTopic, onEditTopic, onDeleteTopic, onCreatePlan, onSubmitPlan }) {
  if (trainings.length === 0) {
  ...
  return (
    <div>
      ...
      {trainings.map((t) => {
        ...
        const locked = !!t.planLockedByLearner;
...
function CreateLearningPlanModal({ training, willLock, onClose, onSave }) {
  ...
  return (
    <Modal title={`Create learning plan — ${training.title}`} onClose={onClose} wide>
      ...
      <Btn style={{ width: "100%", justifyContent: "center", opacity: valid ? 1 : 0.5 }}
        ...
        {willLock ? "Submit plan" : "Save plan"}
      </Btn>
    </Modal>
  );
}
