...
import {
  ...
  ClipboardList, ArrowLeft, Filter, RotateCcw
} from "lucide-react";
import {
  PieChart, Pie, Cell, BarChart, Bar, XAxis, YAxis, Tooltip, Legend,
  ResponsiveContainer, CartesianGrid
} from "recharts";
import { subscribeToKey, writeKey, signUpWithPassword, signInWithPassword, sendPasswordReset, updatePassword, signOutUser, onAuthChange } from "./supabase";
...
export default function App() {
  ...
  if (authEvent === "PASSWORD_RECOVERY") {
    return <SetNewPassword onDone={() => setAuthEvent(null)} />;
  }
  ...
  return (
    <Shell>
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: 560, padding: 40 }}>
        ...
        <h1 style={{ fontFamily: T.serif, fontSize: 26, margin: "0 0 6px", textAlign: "center" }}>Learning Ledger</h1>
...
// ---------- Admin Dashboard ----------
function AdminDashboard({ trainings, users, progress, alerts, onAddTraining, onEnroll, onAddTopic }) {
...
function TrainingCard({ training, users, progress, onEnroll, onAddTopic }) {
...
function TrainingsView({ trainings, users, progress, onAdd, onEnroll, onAddTopic }) {
  if (trainings.length === 0) {
  ...
  return (
    <div>
      ...
      {trainings.map((t) => <TrainingCard key={t.id} training={t} users={users} progress={progress} onEnroll={() => onEnroll(t)} onAddTopic={() => onAddTopic(t)} />)}
    </div>
  );
}

function UsersView({ users, trainings, onAdd, onChangeRole, currentUserId }) {
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 14 }}>
...
function LearnerDashboard({ trainings, progress, currentUser, onUpdate, onAddTopic }) {
  if (trainings.length === 0) {
