...
import { subscribeToKey, writeKey } from "./supabase";
...
function Btn({ children, onClick, variant = "primary", style = {}, ...rest }) {
  ...
  const variants = {
    primary: { background: T.brand, color: "#12151C" },
  ...
  if (!currentUser) {
    return (
      <Login
        ...
        onCreateFirstAdmin={() => setShowFirstAdmin(true)}
        showFirstAdmin={showFirstAdmin}
        closeFirstAdmin={() => setShowFirstAdmin(false)}
        onAddUser={async (u) => {
          const next = [...users, u];
    ...
    <Shell>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            <div style={{ fontFamily: T.serif, fontSize: 18, color: T.ink, lineHeight: 1 }}>Learning Ledger</div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <button onClick={() => setCurrentUser(null)} title="Switch profile" style={{
      ...
      <div style={{ padding: 26, maxWidth: 1100, margin: "0 auto", width: "100%", boxSizing: "border-box" }}>
        ...
        {isAdmin && view === "users" && (
          <UsersView users={users} trainings={trainings} onAdd={() => setShowAddUser(true)} />
        )}
...
// ---------- Login ----------
function Login({ users, onLogin, onAddUser, showFirstAdmin, onCreateFirstAdmin, closeFirstAdmin }) {
  const [name, setName] = useState("");
  const [role, setRole] = useState("admin");
  const [email, setEmail] = useState("");

  if (users.length === 0 && !showFirstAdmin) {
    return (
      <Shell>
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: 560, padding: 40, textAlign: "center" }}>
          <BookOpen size={30} color={T.brand} />
          <h1 style={{ fontFamily: T.serif, fontSize: 26, margin: "16px 0 8px" }}>Learning Ledger</h1>
          <p style={{ color: T.inkMuted, fontSize: 14, maxWidth: 340, marginBottom: 20 }}>
            No workspace yet. Set up the first admin account to add trainings and enroll learners.
          </p>
          <Btn onClick={onCreateFirstAdmin}><Plus size={15} /> Create admin account</Btn>
        </div>
      </Shell>
    );
  }
  ...
  return (
    <Shell>
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: 560, padding: 40 }}>
        <BookOpen size={28} color={T.brand} />
        <h1 style={{ fontFamily: T.serif, fontSize: 24, margin: "14px 0 4px" }}>Learning Ledger</h1>
        <p style={{ color: T.inkFaint, fontSize: 12.5, fontFamily: T.mono, marginBottom: 26 }}>SELECT YOUR PROFILE</p>
        ...
        {!showFirstAdmin ? (
          <div style={{ width: "100%", maxWidth: 360 }}>
            {users.map((u) => (
              <button key={u.id} onClick={() => onLogin(u)} style={{
                ...
                padding: "12px 14px", marginBottom: 8, cursor: "pointer", color: T.ink,
              }}>
                <span style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <span style={{
                    width: 30, height: 30, borderRadius: "50%", background: T.brandDim,
                    display: "flex", alignItems: "center", justifyContent: "center",
                    fontFamily: T.serif, fontSize: 13, color: T.brand,
                  }}>{u.name.charAt(0).toUpperCase()}</span>
                  <span>
                    <div style={{ fontSize: 14, fontWeight: 600, textAlign: "left" }}>{u.name}</div>
                    <div style={{ fontSize: 11, color: T.inkFaint, fontFamily: T.mono, textTransform: "uppercase" }}>{u.role}</div>
                  </span>
                </span>
            ...
            <div style={{ textAlign: "center", marginTop: 14 }}>
              <button onClick={onCreateFirstAdmin} style={{
                background: "none", border: "none", color: T.brand, fontSize: 12.5, cursor: "pointer", fontFamily: T.sans,
              }}>+ Add a profile (admin only, in practice)</button>
            </div>
          </div>
        ) : (
          <div style={{ width: "100%", maxWidth: 340 }}>
            ...
            <Field label="Email (for weekly reports)">
              <input style={inputStyle} value={email} onChange={(e) => setEmail(e.target.value)} placeholder="priya@company.com" />
            </Field>
            <Field label="Role">
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 6 }}>
              {users.length > 0 && <Btn variant="ghost" onClick={closeFirstAdmin}>Cancel</Btn>}
              <Btn onClick={() => name.trim() && onAddUser({ id: uid(), name: name.trim(), email: email.trim(), role })} style={{ flex: 1, justifyContent: "center" }}>
                Continue
              </Btn>
            </div>
          </div>
...
// ---------- Admin Dashboard ----------
function AdminDashboard({ trainings, users, progress, alerts, onAddTraining, onEnroll }) {
  ...
  return (
    <div>
      ...
      {trainings.map((t) => (
        <TrainingCard key={t.id} training={t} users={users} progress={progress} onEnroll={() => onEnroll(t)} />
      ))}
    </div>
  );
}
...
function TrainingCard({ training, users, progress, onEnroll }) {
        ...
        <details style={{ marginTop: 10 }}>
          ...
          <div style={{ marginTop: 6, display: "flex", flexDirection: "column", gap: 4 }}>
            {training.lessonPlan.map((l) => (
              <div key={l.id} style={{ display: "flex", justifyContent: "space-between", fontSize: 11.5, color: T.inkMuted }}>
                <span>{l.topic}</span>
      ...
      {enrolledUsers.length > 0 && (
        <div style={{ marginTop: 14, display: "flex", flexDirection: "column", gap: 10 }}>
          {enrolledUsers.map((u) => {
            ...
            return (
              <div key={u.id} style={{ display: "grid", gridTemplateColumns: "110px 1fr auto", gap: 12, alignItems: "center" }}>
...
function TrainingsView({ trainings, users, progress, onAdd, onEnroll }) {
  if (trainings.length === 0) {
  ...
  return (
    <div>
      ...
      {trainings.map((t) => <TrainingCard key={t.id} training={t} users={users} progress={progress} onEnroll={() => onEnroll(t)} />)}
    </div>
  );
}

function UsersView({ users, trainings, onAdd }) {
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 14 }}>
...
function ReportsView({ trainings, users, progress, onOpenReport }) {
  ...
  trainings.forEach((t) => t.enrolled.forEach((uid_) => {
    ...
    rows.push({ training: t.title, user: u?.name || "—", ...s });
  }));
  ...
  return (
    <div>
      ...
      <SectionHeading icon={ClipboardList} label="Full status table" />
      {rows.length === 0 ? (
        <EmptyState title="Nothing to report yet" body="Enroll learners into a training to see status rows here." />
      ) : (
        <div style={{ background: T.surface, border: `1px solid ${T.line}`, borderRadius: 11, overflow: "hidden" }}>
          <div style={{
...
function LearnerDashboard({ trainings, progress, currentUser, onUpdate }) {
  if (trainings.length === 0) {
  ...
  return (
    <div>
      ...
      {trainings.map((t) => {
        ...
        const doneIds = new Set(progress.filter((p) => p.trainingId === t.id && p.userId === currentUser.id && p.topicId).map((p) => p.topicId));
                  ...
                  {t.lessonPlan.map((l) => {
                    ...
                    return (
                      <div key={l.id} style={{ display: "flex", justifyContent: "space-between", fontSize: 12.5 }}>
                        ...
                        <span style={{ fontFamily: T.mono, color: T.inkFaint }}>{fmtDate(l.expectedDate)}</span>
                      </div>
                    );
                  })}
                </div>
  ...
  return (
    <Modal title="Add a user" onClose={onClose}>
      ...
      <Btn style={{ width: "100%", justifyContent: "center", marginTop: 4 }}
        onClick={() => name.trim() && onSave({ id: uid(), name: name.trim(), email: email.trim(), role })}>
        Add user
      </Btn>
    </Modal>
  );
...
function AddTrainingModal({ onClose, onSave }) {
  ...
  const valid = title.trim() && startDate && cleanLessons.length > 0;

  function handleSave() {
    if (!valid) return;
    const endDate = cleanLessons.reduce((max, l) => (l.expectedDate > max ? l.expectedDate : max), cleanLessons[0].expectedDate);
    onSave({
  ...
  return (
    <Modal title="New training" onClose={onClose} wide>
      ...
      <div style={{ fontSize: 12, color: T.inkMuted, marginBottom: 8, fontFamily: T.sans, fontWeight: 600, letterSpacing: 0.3 }}>
        Lesson plan — topic and expected completion date
      </div>
...
function TopicChecklistModal({ training, currentUser, progress, onClose, onSaveMany }) {
  ...
  function handleSave() {
    // newly checked topics not previously recorded get a completion entry dated today
    const newEntries = [];
    training.lessonPlan.forEach((l) => {
      if (checked.has(l.id) && !existingDone.has(l.id)) {
        newEntries.push({ id: uid(), trainingId: training.id, userId: currentUser.id, topicId: l.id, date: today });
      }
    });
