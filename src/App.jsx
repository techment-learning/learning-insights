...
export default function App() {
  ...
  async function handleDeleteTopic(training, topic, personalUserId) {
    ...
    if (personalUserId) {
      const pp = getPersonalPlan(training, personalUserId, personalPlans);
      if (!pp) return;
      const nextPersonal = personalPlans.map((p) =>
        p.id === pp.id ? { ...p, lessonPlan: p.lessonPlan.filter((l) => l.id !== topic.id) } : p
      );
      const nextProgress = progress.filter((p) => !(p.trainingId === training.id && p.userId === personalUserId && p.topicId === topic.id));
      await persist("ltp-personal-plans", nextPersonal, setPersonalPlans);
  ...
  async function handleCustomizePlan(training, topics) {
    const existing = getPersonalPlan(training, currentUser.id, personalPlans);
    const next = existing
      ? personalPlans.map((p) => p.id === existing.id ? { ...p, lessonPlan: topics, locked: true } : p)
      : [...personalPlans, { id: uid(), trainingId: training.id, userId: currentUser.id, lessonPlan: topics, locked: true }];
    await persist("ltp-personal-plans", next, setPersonalPlans);
    setShowCustomizePlan(null);
    showToast("Your custom plan is saved and locked");
  }

  async function handleRevertToSharedPlan(training) {
    const pp = getPersonalPlan(training, currentUser.id, personalPlans);
    if (!pp) return;
    ...
    const nextPersonal = personalPlans.filter((p) => p.id !== pp.id);
    const nextProgress = progress.filter((p) => !(p.trainingId === training.id && p.userId === currentUser.id && personalTopicIds.has(p.topicId)));
    await persist("ltp-personal-plans", nextPersonal, setPersonalPlans);
    await persist("ltp-progress", nextProgress, setProgress);
        ...
        <AddTopicModal training={showAddTopic.training} onClose={() => setShowAddTopic(null)}
          onSave={async (newTopic) => {
            ...
            if (personalUserId) {
              const pp = getPersonalPlan(training, personalUserId, personalPlans);
              const next = pp
                ? personalPlans.map((p) => p.id === pp.id ? { ...p, lessonPlan: [...p.lessonPlan, newTopic] } : p)
                : [...personalPlans, { id: uid(), trainingId: training.id, userId: personalUserId, lessonPlan: [newTopic], locked: false }];
              await persist("ltp-personal-plans", next, setPersonalPlans);
            } else {
              const next = trainings.map((t) =>
                t.id === training.id ? { ...t, lessonPlan: [...(t.lessonPlan || []), newTopic] } : t
              );
              await persist("ltp-trainings", next, setTrainings);
            }
        ...
        <EditTopicModal training={showEditTopic.training} topic={showEditTopic.topic} onClose={() => setShowEditTopic(null)}
          onSave={async (updatedTopic) => {
            ...
            if (personalUserId) {
              const pp = getPersonalPlan(training, personalUserId, personalPlans);
              if (pp) {
                const next = personalPlans.map((p) =>
...
// ---------- Admin Dashboard ----------
function AdminDashboard({ trainings, users, progress, alerts, personalPlans, onAddTraining, onEnroll, onAddTopic, onEditTopic, onDeleteTopic, onUnenroll, onCreatePlan, onEditTraining, onDeleteTraining }) {
...
function TrainingCard({ training, users, progress, personalPlans, onEnroll, onAddTopic, onEditTopic, onDeleteTopic, onUnenroll, onCreatePlan, onEditTraining, onDeleteTraining }) {
...
function TrainingsView({ trainings, users, progress, personalPlans, onAdd, onEnroll, onAddTopic, onEditTopic, onDeleteTopic, onUnenroll, onCreatePlan, onEditTraining, onDeleteTraining }) {
  if (trainings.length === 0) {
