...
import {
  ...
  ClipboardList, ArrowLeft, Filter, RotateCcw, Pencil, Trash2, UserMinus, Download, Copy, Key, Lock, Unlock, GraduationCap, Search
} from "lucide-react";
import {
  PieChart, Pie, Cell, BarChart, Bar, XAxis, YAxis, Tooltip, Legend,
  ResponsiveContainer, CartesianGrid
} from "recharts";
import { subscribeToKey, writeKey, readKey, signUpWithPassword, signInWithPassword, signOutUser, onAuthChange, addUserWithPasskey, regeneratePasskey, recoverAdminAccess } from "./api";
import { TECHMENT_LOGO } from "./logo";
import * as XLSX from "xlsx";

// Domain enforcement now happens server-side, in the Worker's /api/auth/signup
// handler — a client-side-only check here couldn't stop a direct API call.
...
const T = {
  ...
  inkMuted: "#5B6472",
  inkFaint: "#8891A0",
  // Techment brand colors (from techment.com)
  brand: "#3F79DA",
  brandDim: "#E8EFFC",
  brandGreen: "#3AE8AD",
  brandGreenDim: "#E5FBF3",
  onTrack: "#0EA66E",
  onTrackDim: "#E3FBF1",
  // Was a muddy amber-brown that read almost identically to "delayed" red
  // at a glance — violet is unmistakably distinct from both red and green,
  // including for red-green colorblind viewers.
  progress: "#8B5CF6",
  progressDim: "#F1ECFE",
  delayed: "#E5484D",
  delayedDim: "#FDECEC",
  serif: "'Sora', -apple-system, 'Segoe UI', sans-serif",
...
// Client-side hint only, for fast feedback before hitting the server — the
// real, enforced check lives server-side in the Worker's signup handler,
// and this must be kept in sync with that if you change your domain.
const ALLOWED_EMAIL_DOMAIN_HINT = "techment.com";
...
function isPlanLocked(training, userId, personalPlans) {
  const pp = getPersonalPlan(training, userId, personalPlans);
  return pp ? !!pp.locked : !!training.planLocked;
}
...
// Self-learning status is purely a function of the dates and the entered
// completion percentage — no separate progress log behind it, unlike the
// assigned-training statuses above.
//   Completed: reached 100%, regardless of dates.
//   Dropped: deadline has passed and literally no progress was ever made.
//   Delayed: behind pace for today, or past deadline with partial progress.
//   On Track: at or ahead of the pace expected for today.
function computeSelfLearningStatus(entry) {
  const pct = entry.percentCompleted ?? 0;
  if (pct >= 100) return "Completed";

  const today = todayISO();
  if (today > entry.endDate) return pct === 0 ? "Dropped" : "Delayed";

  const totalDays = daysBetween(entry.startDate, entry.endDate);
  const elapsedDays = daysBetween(entry.startDate, today);
  const plannedPct = totalDays > 0 ? Math.max(0, Math.min(100, (elapsedDays / totalDays) * 100)) : 0;
  return pct >= plannedPct ? "On Track" : "Delayed";
}

const SELF_LEARNING_STATUS_STYLE = {
  "On Track": { color: T.onTrack, bg: T.onTrackDim },
  Delayed: { color: T.delayed, bg: T.delayedDim },
  Completed: { color: T.brand, bg: T.brandDim },
  Dropped: { color: T.inkFaint, bg: T.surface },
};
...
export default function App() {
  ...
  const [regeneratedPasskey, setRegeneratedPasskey] = useState(null); // { user, passkey } | null
  ...
  // Only start pulling shared data once someone's actually signed in — the
  // Worker rejects unauthenticated requests anyway, and there's no separate
  // "reconcile auth with a user list" step needed here: sign-in and sign-up
  // already return the complete, correct user object directly from the
  // database that's the single source of truth for both identity and role.
  useEffect(() => {
    if (!currentUser) { setLoaded(false); return; }
    let gotUsers = false, gotTrainings = false, gotProgress = false, gotPersonalPlans = false, gotSelfLearning = currentUser.role !== "admin";
    const markLoaded = () => {
      if (gotUsers && gotTrainings && gotProgress && gotPersonalPlans && gotSelfLearning) setLoaded(true);
    };
    ...
    // Self-learning is an admin-only management tool — the Worker refuses
    // reads for anyone else, so only subscribe here if that won't just be
    // a repeated failed request every poll cycle.
    const unsubSelfLearning = currentUser.role === "admin"
      ? subscribeToKey("ltp-self-learning", (v) => { setSelfLearning(v); gotSelfLearning = true; markLoaded(); })
      : () => {};
    return () => { unsubUsers(); unsubTrainings(); unsubProgress(); unsubPersonalPlans(); unsubSelfLearning(); };
  }, [currentUser]);
  ...
  async function handleCustomizePlan(training, topics) {
    await persist("ltp-personal-plans", (fresh) => {
      ...
      return existing
        ? fresh.map((p) => p.id === existing.id ? { ...p, lessonPlan: topics } : p)
        : [...fresh, { id: uid(), trainingId: training.id, userId: currentUser.id, lessonPlan: topics, locked: false }];
    }, setPersonalPlans);
    setShowCustomizePlan(null);
    showToast("Your custom plan is saved");
  }
  ...
  async function handleCloneTraining(training, newTitle, newStartDate, newEndDate) {
    ...
    const newTraining = {
      ...
      lessonPlan: shiftedTopics, enrolled: [], planLocked: false,
    };
  ...
  async function handleCreatePlan(training, topics, willLock) {
    await persist("ltp-trainings", (fresh) =>
      fresh.map((t) => t.id === training.id ? { ...t, lessonPlan: topics, planLocked: willLock } : t),
      setTrainings);
    setShowCreatePlan(null);
    showToast(willLock ? "Learning plan submitted and locked" : "Learning plan saved");
  }

  // Locking is admin-only: enrollment always starts unlocked, and only an
  // admin can toggle it from here on — enforced both here and server-side.
  async function handleToggleTrainingLock(training) {
    const next = !training.planLocked;
    await persist("ltp-trainings", (fresh) => fresh.map((t) => t.id === training.id ? { ...t, planLocked: next } : t), setTrainings);
    showToast(next ? "Plan locked — only you can edit it now" : "Plan unlocked — the learner can edit it again");
  }

  async function handleTogglePersonalPlanLock(personalPlan) {
    const next = !personalPlan.locked;
    await persist("ltp-personal-plans", (fresh) => fresh.map((p) => p.id === personalPlan.id ? { ...p, locked: next } : p), setPersonalPlans);
    showToast(next ? "Custom plan locked — only you can edit it now" : "Custom plan unlocked");
  }

  async function handleAddSelfLearning(entry) {
    await persist("ltp-self-learning", (fresh) => [...fresh, { ...entry, id: uid() }], setSelfLearning);
    showToast(`${entry.personName}'s entry added`);
  }

  async function handleEditSelfLearning(id, updates) {
    await persist("ltp-self-learning", (fresh) => fresh.map((s) => s.id === id ? { ...s, ...updates } : s), setSelfLearning);
    showToast("Entry updated");
  }

  async function handleDeleteSelfLearning(entry) {
    if (!window.confirm(`Delete ${entry.personName}'s "${entry.detail}" entry? This can't be undone.`)) return;
    await persist("ltp-self-learning", (fresh) => fresh.filter((s) => s.id !== entry.id), setSelfLearning);
    showToast("Entry deleted");
  }

  async function handleToggleSelfSkillUpdate(entry) {
    if (entry.percentCompleted < 100) return; // only togglable once complete — enforced server-side too
    const next = !entry.selfSkillUpdated;
    await persist("ltp-self-learning", (fresh) => fresh.map((s) => s.id === entry.id ? { ...s, selfSkillUpdated: next } : s), setSelfLearning);
  }
  ...
  async function handleDeleteUser(user) {
    ...
    // The Worker's delete handler cascades this in one call: removes them
    // from every training's enrolled list, deletes their progress and
    // personal plans, and clears their sessions — no separate client-side
    // cleanup calls needed.
  ...
  if (!currentUser) {
    return (
      <Login
        ...
        onRecoverAdmin={(email, pin, newPasskey) => recoverAdminAccess(email, pin, newPasskey)}
        error={authError}
      />
    );
  }
  ...
  return (
    <Shell>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{
            ...
            <img src={TECHMENT_LOGO} alt="Techment" style={{ width: 26, height: 26, objectFit: "contain" }} />
          </div>
          <div>
      ...
      <div style={{ display: "flex", gap: 4, padding: "12px 26px 0", borderBottom: `1px solid ${T.line}` }}>
        {(isAdmin
          ? [["dashboard", "Dashboard", TrendingUp], ["trainings", "Trainings", BookOpen], ["users", "Users", Users], ["selfLearning", "Self Learning", GraduationCap], ["reports", "Reports", FileText]]
      ...
      <div style={{ padding: 26, maxWidth: 1100, margin: "0 auto", width: "100%", boxSizing: "border-box" }}>
        {isAdmin && view === "dashboard" && (
          <AdminDashboard trainings={trainings} users={users} progress={progress} alerts={alerts} personalPlans={personalPlans} selfLearning={selfLearning}
            ...
            onToggleLock={(t) => handleToggleTrainingLock(t)}
            onTogglePersonalLock={(pp) => handleTogglePersonalPlanLock(pp)} />
        )}
        ...
        {isAdmin && view === "users" && (
          <UsersView users={users} trainings={trainings} onAdd={() => setShowAddUser(true)} currentUserId={currentUser.id}
            ...
            onRegeneratePasskey={async (u) => {
              if (!window.confirm(`Generate a new passkey for ${u.name}? Their current passkey stops working immediately.`)) return;
              try {
                const result = await regeneratePasskey(u.id);
                setRegeneratedPasskey({ user: u, passkey: result.passkey });
              } catch (e) {
                console.error(e);
                showToast(e.message || "Could not generate a new passkey", true);
              }
            }}
            onDeleteUser={handleDeleteUser} />
        )}
        {isAdmin && view === "selfLearning" && (
          <SelfLearningView entries={selfLearning}
            onAdd={handleAddSelfLearning}
            onEdit={handleEditSelfLearning}
            onDelete={handleDeleteSelfLearning}
            onToggleSelfSkillUpdate={handleToggleSelfSkillUpdate} />
        )}
      ...
      {showAddUser && (
        <AddUserModal onClose={() => setShowAddUser(false)} onSave={(u) => {
          // The Worker's add-user endpoint already created this row
          // server-side (with its passkey) — this just reflects it in the
          // UI immediately instead of waiting for the next poll cycle.
          setUsers((fresh) => [...fresh, u]);
          showToast(`${u.name} added`);
        }} />
      )}

      {regeneratedPasskey && (
        <Modal title="New passkey generated" onClose={() => setRegeneratedPasskey(null)}>
          <p style={{ fontSize: 12.5, color: T.inkMuted, marginBottom: 10 }}>
            Share this with {regeneratedPasskey.user.name} directly (Teams, WhatsApp, in person) — it's shown only this once and their old passkey no longer works.
          </p>
          <div style={{
            background: T.brandDim, border: `1px solid ${T.line}`, borderRadius: 8,
            padding: "14px 16px", marginBottom: 16, textAlign: "center",
          }}>
            <div style={{ fontFamily: "monospace", fontSize: 20, fontWeight: 700, letterSpacing: 1.5, color: T.ink }}>{regeneratedPasskey.passkey}</div>
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            <Btn variant="ghost" onClick={() => setRegeneratedPasskey(null)} style={{ flex: 1, justifyContent: "center" }}>Done</Btn>
            <Btn onClick={() => navigator.clipboard.writeText(regeneratedPasskey.passkey)} style={{ flex: 1, justifyContent: "center" }}>Copy passkey</Btn>
          </div>
        </Modal>
      )}
...
// ---------- Login ----------
function Login({ onSignIn, onSignUp, onRecoverAdmin, error }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      if (mode === "signin") {
        ...
        await onRecoverAdmin(email.trim(), pin, password);
      } else {
        if (ALLOWED_EMAIL_DOMAIN_HINT && !email.trim().toLowerCase().endsWith(`@${ALLOWED_EMAIL_DOMAIN_HINT}`)) {
          throw new Error(`Please use your official @${ALLOWED_EMAIL_DOMAIN_HINT} email.`);
        }
        // No email confirmation step in this backend — signing up logs you
        // straight in, so there's nothing further to prompt here. This only
        // succeeds at all when the workspace has no admin yet — the server
        // enforces that; everyone after the first admin gets added BY that
        // admin, who hands them a passkey directly (never self-signup).
  ...
  return (
    <Shell>
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: 560, padding: 40 }}>
        <div style={{
          width: 64, height: 64, borderRadius: 14, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 18,
        }}>
          <img src={TECHMENT_LOGO} alt="Techment" style={{ width: 58, height: 58, objectFit: "contain" }} />
        </div>
        ...
        <p style={{ color: T.inkMuted, fontSize: 13, marginBottom: 24, textAlign: "center" }}>
          {mode === "signin" && "Sign in with the email and passkey your admin gave you."}
          {mode === "signup" && "First-time setup — only works if no admin exists yet."}
          {mode === "recover" && "Admin recovery — requires the recovery pin, set up separately in the Worker."}
        </p>
        ...
        <form onSubmit={handleSubmit} style={{ width: "100%", maxWidth: 340 }}>
          ...
          {mode === "recover" && (
            <Field label="Recovery pin">
              <input type="password" style={inputStyle} value={pin} onChange={(e) => setPin(e.target.value)} placeholder="The pin set in the Worker's settings" required />
            </Field>
          )}
          <Field label={mode === "signin" ? "Passkey" : mode === "recover" ? "New passkey" : "Choose a password"}>
          ...
          <Btn type="submit" disabled={busy} style={{ width: "100%", justifyContent: "center", opacity: busy ? 0.7 : 1, marginTop: 4 }}>
            {busy ? "Please wait…" : mode === "signin" ? "Sign in" : mode === "recover" ? "Reset admin passkey" : "Set up workspace"}
          </Btn>
        </form>

        <div style={{ marginTop: 18, fontSize: 12.5, color: T.inkMuted, textAlign: "center" }}>
          {mode === "signin" && (
            <>
              <div>Lost your passkey? Ask your admin to generate you a new one.</div>
              <div style={{ marginTop: 8, display: "flex", gap: 12, justifyContent: "center" }}>
                <button onClick={() => { setMode("signup"); setFormError(null); }} style={{ background: "none", border: "none", color: T.inkFaint, cursor: "pointer", fontFamily: T.sans, fontSize: 11, textDecoration: "underline" }}>First-time setup</button>
                <button onClick={() => { setMode("recover"); setFormError(null); }} style={{ background: "none", border: "none", color: T.inkFaint, cursor: "pointer", fontFamily: T.sans, fontSize: 11, textDecoration: "underline" }}>Admin locked out?</button>
              </div>
            </>
          )}
          {(mode === "signup" || mode === "recover") && (
            <span>Already set up? <button onClick={() => { setMode("signin"); setFormError(null); }} style={{ background: "none", border: "none", color: T.brand, cursor: "pointer", fontFamily: T.sans, fontSize: 12.5 }}>Sign in</button></span>
          )}
        </div>

        <p style={{ color: T.inkFaint, fontSize: 11, marginTop: 22, maxWidth: 300, textAlign: "center" }}>
          Only the very first person here sets up their own access. Everyone after that is added by an admin, who shares a passkey with them directly.
        </p>
      </div>
    </Shell>
  );
...
// ---------- Admin Dashboard ----------
function AdminDashboard({ trainings, users, progress, alerts, personalPlans, selfLearning, onAddTraining, onEnroll, onAddTopic, onEditTopic, onDeleteTopic, onUnenroll, onCreatePlan, onEditTraining, onDeleteTraining, onCloneTraining, onToggleLock, onTogglePersonalLock }) {
  ...
  const slCounts = { "On Track": 0, Delayed: 0, Completed: 0, Dropped: 0 };
  selfLearning.forEach((e) => { slCounts[computeSelfLearningStatus(e)]++; });

  const selfLearningSummary = selfLearning.length > 0 && (
    <div style={{ marginBottom: 26 }}>
      <SectionHeading icon={GraduationCap} label="Self learning" />
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(120px, 1fr))", gap: 10 }}>
        <StatCard label="Total Upskilling" value={selfLearning.length} color={T.ink} />
        <StatCard label="On Track" value={slCounts["On Track"]} color={T.onTrack} />
        <StatCard label="Delayed" value={slCounts.Delayed} color={T.delayed} />
        <StatCard label="Completed" value={slCounts.Completed} color={T.brand} />
        <StatCard label="Dropped" value={slCounts.Dropped} color={T.inkFaint} />
      </div>
    </div>
  );
    ...
    <div>
      ...
      {trainings.map((t) => (
        <TrainingCard key={t.id} training={t} users={users} progress={progress} personalPlans={personalPlans} onEnroll={() => onEnroll(t)}
          ...
          onToggleLock={onToggleLock} onTogglePersonalLock={onTogglePersonalLock} />
      ))}
    </div>
  );
}
...
function TrainingCard({ training, users, progress, personalPlans, onEnroll, onAddTopic, onEditTopic, onDeleteTopic, onUnenroll, onCreatePlan, onEditTraining, onDeleteTraining, onCloneTraining, onToggleLock, onTogglePersonalLock }) {
    ...
    <div style={{ background: T.surface, border: `1px solid ${T.line}`, borderRadius: 11, padding: 18, marginBottom: 12 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 10 }}>
        <div style={{ display: "flex", alignItems: "flex-start", gap: 8 }}>
          <div>
            ...
            <div style={{ fontFamily: T.mono, fontSize: 11.5, color: T.inkFaint, marginTop: 4 }}>
              ...
              {training.planLocked && <span style={{ color: T.brand }}> · shared plan locked</span>}
            </div>
          </div>
          {onEditTraining && (
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          ...
          {hasTopics && onToggleLock && (
            <Btn variant="ghost" onClick={() => onToggleLock(training)}>
              {training.planLocked ? <><Lock size={13} /> Unlock plan</> : <><Unlock size={13} /> Lock plan</>}
            </Btn>
          )}
              ...
              <div key={u.id}>
                ...
                {isCustom && (
                  <details style={{ marginTop: 4, marginLeft: 2 }}>
                    <summary style={{ fontSize: 11, color: T.brand, cursor: "pointer" }}>
                      {u.name}'s custom plan ({personalPlan.lessonPlan.length} topics){personalPlan.locked && " · locked"}
                    </summary>
                    {onTogglePersonalLock && (
                      <button onClick={() => onTogglePersonalLock(personalPlan)} style={{
                        display: "flex", alignItems: "center", gap: 4, marginTop: 6, marginLeft: 8,
                        background: "none", border: "none", color: T.inkFaint, cursor: "pointer", fontFamily: T.sans, fontSize: 11, padding: 0,
                      }}>
                        {personalPlan.locked ? <><Lock size={11} /> Unlock this plan</> : <><Unlock size={11} /> Lock this plan</>}
                      </button>
                    )}
...
function TrainingsView({ trainings, users, progress, personalPlans, onAdd, onEnroll, onAddTopic, onEditTopic, onDeleteTopic, onUnenroll, onCreatePlan, onEditTraining, onDeleteTraining, onCloneTraining, onToggleLock, onTogglePersonalLock }) {
  if (trainings.length === 0) {
...
function UsersView({ users, trainings, onAdd, onChangeRole, onDeleteUser, onRegeneratePasskey, currentUserId }) {
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 14 }}>
            ...
            return (
              <div key={u.id} style={{
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                  ...
                  <button onClick={() => onRegeneratePasskey(u)} title={`Generate a new passkey for ${u.name}`} style={{
                    background: "none", border: "none", color: T.inkFaint, cursor: "pointer", padding: 2, display: "flex",
                  }}>
                    <Key size={14} />
                  </button>
...
function SelfLearningView({ entries, onAdd, onEdit, onDelete, onToggleSelfSkillUpdate }) {
  const [showModal, setShowModal] = useState(false); // false | true (add) | entry object (edit)
  const [search, setSearch] = useState("");
  const [quarterFilter, setQuarterFilter] = useState("all");

  const counts = { "On Track": 0, Delayed: 0, Completed: 0, Dropped: 0 };
  entries.forEach((e) => { counts[computeSelfLearningStatus(e)]++; });
  ...
  const filteredEntries = entries
    .filter((e) => {
      ...
      if (!q) return true;
      return e.personName.toLowerCase().includes(q) ||
        e.detail.toLowerCase().includes(q) ||
        (e.managerName || "").toLowerCase().includes(q) ||
        e.quarter.toLowerCase().includes(q);
    })
    // Group each person's multiple courses together, most recent start
    // first within that group — makes it easy to scan someone's whole
    // upskilling picture (what's done, what's still in progress) at once.
    .sort((a, b) => a.personName.localeCompare(b.personName) || b.startDate.localeCompare(a.startDate));

  function downloadExcel() {
    const rows = filteredEntries.map((e) => ({
      Name: e.personName,
      "Learning Type": e.learningType,
      "Course / Certification": e.detail,
      "Start Date": e.startDate,
      "End Date": e.endDate,
      "% Completed": e.percentCompleted,
      Status: computeSelfLearningStatus(e),
      "Assigned By": e.assignedBy,
      "Manager Name": e.managerName || "",
      Remark: e.remark || "",
      Quarter: e.quarter,
      "Self Skill Update": computeSelfLearningStatus(e) === "Completed" && e.selfSkillUpdated ? "Done" : "Not done",
    }));
    const worksheet = XLSX.utils.json_to_sheet(rows);
    worksheet["!cols"] = [{ wch: 20 }, { wch: 13 }, { wch: 30 }, { wch: 12 }, { wch: 12 }, { wch: 11 }, { wch: 11 }, { wch: 12 }, { wch: 18 }, { wch: 30 }, { wch: 22 }, { wch: 16 }];
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "Self Learning");
    const label = quarterFilter === "all" ? "all-quarters" : quarterFilter.split(" ")[0] + "-" + quarterFilter.split(" ")[1];
    XLSX.writeFile(workbook, `self-learning-${label}.xlsx`);
  }

  return (
    <div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(120px, 1fr))", gap: 10, marginBottom: 20 }}>
        <StatCard label="Total Upskilling" value={entries.length} color={T.ink} />
        ...
        <StatCard label="Dropped" value={counts.Dropped} color={T.inkFaint} />
      </div>

      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14, gap: 12, flexWrap: "wrap" }}>
        <p style={{ fontSize: 12.5, color: T.inkMuted, maxWidth: 400, margin: 0 }}>
          Track self-driven courses and certifications — separate from the assigned training plans. Status is calculated automatically from the dates and % completed.
        </p>
        <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
          <select value={quarterFilter} onChange={(e) => setQuarterFilter(e.target.value)} style={{ ...inputStyle, width: 200, marginBottom: 0, cursor: "pointer" }}>
            <option value="all">All quarters</option>
            {QUARTER_OPTIONS.map((qOpt) => <option key={qOpt} value={qOpt}>{qOpt}</option>)}
          </select>
          <div style={{ position: "relative" }}>
            <Search size={14} color={T.inkFaint} style={{ position: "absolute", left: 10, top: "50%", transform: "translateY(-50%)" }} />
            <input
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              placeholder="Search name, course, manager…"
              style={{ ...inputStyle, width: 220, paddingLeft: 30, marginBottom: 0 }}
            />
          </div>
          <Btn variant="ghost" onClick={downloadExcel} disabled={filteredEntries.length === 0} style={{ opacity: filteredEntries.length === 0 ? 0.5 : 1 }}>
            <Download size={14} /> Download Excel
          </Btn>
          <Btn onClick={() => setShowModal(true)}><Plus size={14} /> Add entry</Btn>
        </div>
      </div>

      {entries.length === 0 ? (
        <EmptyState title="No self-learning entries yet" body="Add someone's course or certification to start tracking it."
          action={<Btn onClick={() => setShowModal(true)}><Plus size={15} /> Add an entry</Btn>} />
      ) : filteredEntries.length === 0 ? (
        <EmptyState title="No matches" body={search ? `Nothing matches "${search}" in this quarter.` : "No entries in this quarter yet."} />
      ) : (
        <div style={{ background: T.surface, border: `1px solid ${T.line}`, borderRadius: 11, overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12.5, minWidth: 900 }}>
            <thead>
              <tr style={{ borderBottom: `1px solid ${T.line}`, textAlign: "left" }}>
                {["Name", "Learning", "Dates", "%", "Status", "Assigned by", "Quarter", "Remark", "Self Skill Update", ""].map((h) => (
                  <th key={h} style={{ padding: "10px 12px", fontFamily: T.mono, fontSize: 10.5, textTransform: "uppercase", color: T.inkFaint, fontWeight: 600, whiteSpace: "nowrap" }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {filteredEntries.map((entry, i) => {
                const status = computeSelfLearningStatus(entry);
                const style = SELF_LEARNING_STATUS_STYLE[status];
                const samePersonAsPrev = i > 0 && filteredEntries[i - 1].personName === entry.personName;
                return (
                  <tr key={entry.id} style={{
                    borderBottom: i < filteredEntries.length - 1 ? `1px solid ${T.line}` : "none",
                    borderTop: samePersonAsPrev ? "none" : (i > 0 ? `2px solid ${T.line}` : "none"),
                  }}>
                    <td style={{ padding: "10px 12px", fontWeight: 600, whiteSpace: "nowrap", color: samePersonAsPrev ? T.inkFaint : T.ink }}>
                      {samePersonAsPrev ? "↳" : entry.personName}
                    </td>
                    <td style={{ padding: "10px 12px", maxWidth: 220 }}>
                      <div style={{ fontFamily: T.mono, fontSize: 10, color: T.inkFaint, textTransform: "uppercase" }}>{entry.learningType}</div>
                      <div>{entry.detail}</div>
                    </td>
                    <td style={{ padding: "10px 12px", fontFamily: T.mono, fontSize: 11.5, whiteSpace: "nowrap", color: T.inkMuted }}>
                      {fmtDate(entry.startDate)} → {fmtDate(entry.endDate)}
                    </td>
                    <td style={{ padding: "10px 12px", fontFamily: T.mono, whiteSpace: "nowrap" }}>{entry.percentCompleted}%</td>
                    ...
                    <td style={{ padding: "10px 12px", whiteSpace: "nowrap" }}>
                      {entry.assignedBy}{entry.managerName ? ` (${entry.managerName})` : ""}
                    </td>
                    <td style={{ padding: "10px 12px", fontFamily: T.mono, fontSize: 11, whiteSpace: "nowrap", color: T.inkMuted }}>{entry.quarter}</td>
                    <td style={{ padding: "10px 12px", maxWidth: 160, color: T.inkMuted }}>{entry.remark || "—"}</td>
                    <td style={{ padding: "10px 12px", textAlign: "center" }}>
                      <input
                        type="checkbox"
                        checked={status === "Completed" && !!entry.selfSkillUpdated}
                        disabled={status !== "Completed"}
                        onChange={() => onToggleSelfSkillUpdate(entry)}
                        title={status === "Completed" ? "Mark self skill profile as updated" : "Only available once the course/certification reaches 100%"}
                        style={{ cursor: status === "Completed" ? "pointer" : "not-allowed", width: 16, height: 16 }}
                      />
                    </td>
                    <td style={{ padding: "10px 12px", whiteSpace: "nowrap" }}>
                      <div style={{ display: "flex", gap: 6 }}>
                        <button onClick={() => setShowModal({ duplicateOf: entry })} title={`Log another course for ${entry.personName}`} style={{ background: "none", border: "none", color: T.inkFaint, cursor: "pointer", padding: 2, display: "flex" }}>
                          <Copy size={13} />
                        </button>
                        <button onClick={() => setShowModal(entry)} title="Edit" style={{ background: "none", border: "none", color: T.inkFaint, cursor: "pointer", padding: 2, display: "flex" }}>
                          <Pencil size={13} />
                        </button>
                        <button onClick={() => onDelete(entry)} title="Delete" style={{ background: "none", border: "none", color: T.delayed, cursor: "pointer", padding: 2, display: "flex" }}>
                          <Trash2 size={13} />
                        </button>
                      </div>
                    </td>
      ...
      {showModal && (
        <SelfLearningEntryModal
          entry={showModal === true || showModal.duplicateOf ? null : showModal}
          prefill={showModal.duplicateOf}
          onClose={() => setShowModal(false)}
          onSave={(data) => {
            if (showModal === true || showModal.duplicateOf) onAdd(data);
            else onEdit(showModal.id, data);
            setShowModal(false);
          }}
        />
      )}
...
function ReportsView({ trainings, users, progress, personalPlans, onOpenReport }) {
  ...
  // Empty set = no filter applied (show everything, chips start unhighlighted).
  // Clicking a chip adds it to the filter — filtering TO what you click,
  // rather than needing to deselect every other status first.
  ...
  const filtersActive = trainingFilter !== "all" || learnerFilter !== "all" || statusFilter.size > 0;
  const resetFilters = () => { setTrainingFilter("all"); setLearnerFilter("all"); setStatusFilter(new Set()); };
    ...
    <div>
      ...
      <div style={{ background: T.surface, border: `1px solid ${T.line}`, borderRadius: 11, padding: 16, marginBottom: 20 }}>
        ...
        <div style={{ paddingTop: 12 }}>
          ...
          {dlLearnerId && dlLearnerPlan.length > 0 && (
            <div style={{ marginTop: 12, border: `1px solid ${T.line}`, borderRadius: 8, overflow: "hidden" }}>
              {dlLearnerPlan.map((l, i) => {
                const entry = progress.find((p) => p.trainingId === dlTraining.id && p.userId === dlLearnerId && p.topicId === l.id);
                const done = !!entry;
                return (
                  <div key={l.id} style={{
                    ...
                    borderBottom: i < dlLearnerPlan.length - 1 ? `1px solid ${T.line}` : "none",
                    background: done ? T.onTrackDim : "transparent",
                  }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, minWidth: 0 }}>
                      {done ? <CheckCircle2 size={14} color={T.onTrack} style={{ flexShrink: 0 }} /> : <Clock size={14} color={T.inkFaint} style={{ flexShrink: 0 }} />}
                      <span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{l.topic}</span>
                    </div>
                    <div style={{ display: "flex", alignItems: "center", gap: 10, flexShrink: 0, fontFamily: T.mono, fontSize: 11, color: T.inkFaint }}>
                      {done ? <span style={{ color: T.onTrack }}>Done {fmtDate(entry.date)}</span> : <span>Due {fmtDate(l.expectedDate)}</span>}
                    </div>
                  </div>
                );
              })}
...
function LearnerDashboard({ trainings, progress, currentUser, personalPlans, onUpdate, onAddTopic, onEditTopic, onDeleteTopic, onCreatePlan, onCustomizePlan, onRevertToSharedPlan }) {
  if (trainings.length === 0) {
  ...
  return (
    <div>
      ...
      {trainings.map((t) => {
        ...
        const locked = isCustom ? !!personalPlan.locked : !!t.planLocked;
  ...
  return (
    <Modal title={isCustomize ? `Customize your plan — ${training.title}` : `Create learning plan — ${training.title}`} onClose={onClose} wide>
      <p style={{ fontSize: 12.5, color: T.inkMuted, marginBottom: 6, lineHeight: 1.5 }}>
        {isCustomize
          ? "This starts as a copy of the admin's shared plan — edit, add, or remove topics freely. Once saved, it's yours alone: other learners keep following the shared plan untouched. Your admin can lock it later once it's ready, after which only they can change it."
        ...
        {!isCustomize && " You can edit or delete any of these later — until your admin locks the plan, after which only they can change it."}
      </p>
      {isCustomize && (
      ...
      <Btn style={{ width: "100%", justifyContent: "center", opacity: valid ? 1 : 0.5 }}
        ...
        {isCustomize ? "Save my custom plan" : "Save plan"}
      </Btn>
    </Modal>
  );
}
...
function AddUserModal({ onClose, onSave }) {
  ...
  async function handleAdd() {
    ...
    setError(null);
    try {
      const result = await addUserWithPasskey(email.trim(), name.trim(), role);
      setSaved(result.user);
      setPasskey(result.passkey);
      onSave(result.user);
    } catch (e) {
      console.error(e);
      setError(e.message || "Could not add this user.");
    } finally {
      setBusy(false);
    }
  }
  ...
  if (saved) {
    return (
      <Modal title="User added" onClose={onClose}>
        ...
        <p style={{ fontSize: 12.5, color: T.inkMuted, marginBottom: 10 }}>
          Share this passkey with them directly (Teams, WhatsApp, in person) — it's shown only this once and can't be retrieved again. They'll sign in with their email and this passkey.
        </p>

        <div style={{
          background: T.surfaceMuted || T.brandDim, border: `1px solid ${T.line}`, borderRadius: 8,
          padding: "14px 16px", marginBottom: 16, textAlign: "center",
        }}>
          <div style={{ fontFamily: "monospace", fontSize: 20, fontWeight: 700, letterSpacing: 1.5, color: T.ink }}>{passkey}</div>
        </div>

        <div style={{ display: "flex", gap: 8 }}>
          <Btn variant="ghost" onClick={onClose} style={{ flex: 1, justifyContent: "center" }}>Done</Btn>
          <Btn onClick={() => { navigator.clipboard.writeText(passkey); }} style={{ flex: 1, justifyContent: "center" }}>Copy passkey</Btn>
        </div>
      </Modal>
    );
  }
  ...
  return (
    <Modal title="Add a user" onClose={onClose}>
      ...
      <p style={{ fontSize: 11, color: T.inkFaint, marginTop: -10, marginBottom: 16 }}>
        A random passkey will be generated for them — you'll share it directly, no email is sent.
      </p>
      {error && (
        <div style={{
          background: T.delayedDim, border: `1px solid ${T.delayed}44`, color: T.delayed,
      ...
      <Btn style={{ width: "100%", justifyContent: "center", marginTop: 4, opacity: busy ? 0.7 : 1 }} onClick={handleAdd} disabled={busy}>
...
function generateQuarterOptions(count = 12) {
  const quarterMonths = [["Apr", "Jun"], ["Jul", "Sep"], ["Oct", "Dec"], ["Jan", "Mar"]];
  const options = [];
  let qIndex = 0, fyStartYear = 2026; // Q1 FY26-27 = Apr-Jun 2026
  for (let i = 0; i < count; i++) {
    const [m1, m2] = quarterMonths[qIndex];
    const calYear = qIndex === 3 ? fyStartYear + 1 : fyStartYear;
    options.push(`Q${qIndex + 1} FY${String(fyStartYear).slice(2)}-${String(fyStartYear + 1).slice(2)} (${m1}-${m2} ${calYear})`);
    qIndex++;
    if (qIndex === 4) { qIndex = 0; fyStartYear++; }
  }
  return options;
}
const QUARTER_OPTIONS = generateQuarterOptions();

// Derives the same fiscal-quarter label format from any date — used to
// auto-select the quarter from the target completion (end) date, instead
// of a manual dropdown. Fiscal year runs Apr-Mar (Q1=Apr-Jun, ... Q4=Jan-Mar).
function quarterFromDate(dateStr) {
  if (!dateStr) return QUARTER_OPTIONS[0];
  const d = new Date(dateStr + "T00:00:00");
  const month = d.getMonth(); // 0-11
  const year = d.getFullYear();
  const quarterMonths = [["Apr", "Jun"], ["Jul", "Sep"], ["Oct", "Dec"], ["Jan", "Mar"]];
  let qIndex, fyStartYear;
  if (month >= 3 && month <= 5) { qIndex = 0; fyStartYear = year; }
  else if (month >= 6 && month <= 8) { qIndex = 1; fyStartYear = year; }
  else if (month >= 9 && month <= 11) { qIndex = 2; fyStartYear = year; }
  else { qIndex = 3; fyStartYear = year - 1; } // Jan-Mar belongs to the FY that started the previous April
  const [m1, m2] = quarterMonths[qIndex];
  const calYear = qIndex === 3 ? fyStartYear + 1 : fyStartYear;
  return `Q${qIndex + 1} FY${String(fyStartYear).slice(2)}-${String(fyStartYear + 1).slice(2)} (${m1}-${m2} ${calYear})`;
}

function SelfLearningEntryModal({ entry, prefill, onClose, onSave }) {
  const isEdit = !!entry;
  const selectStyle = { ...inputStyle, cursor: "pointer" };
  // Duplicating someone's entry carries over who they are and who
  // assigned it, but never the course-specific details (detail, dates,
  // percent, remark) — those genuinely start fresh for a new course.
  const [personName, setPersonName] = useState(entry?.personName || prefill?.personName || "");
  const [learningType, setLearningType] = useState(entry?.learningType || "Course");
  ...
  const [percentCompleted, setPercentCompleted] = useState(entry?.percentCompleted ?? 0);
  const [assignedBy, setAssignedBy] = useState(entry?.assignedBy || prefill?.assignedBy || "Self");
  const [managerName, setManagerName] = useState(entry?.managerName || prefill?.managerName || "");
  const [remark, setRemark] = useState(entry?.remark || "");
  // Quarter is derived automatically from the target completion (end) date,
  // not chosen manually — editing the date always keeps it in sync.
  const quarter = quarterFromDate(endDate);

  const valid = personName.trim() && detail.trim() && startDate && endDate && endDate >= startDate;

  function handleSave() {
    if (!valid) return;
    onSave({
      personName: personName.trim(), learningType, detail: detail.trim(),
      startDate, endDate, percentCompleted: Math.max(0, Math.min(100, Number(percentCompleted) || 0)),
      assignedBy, managerName: managerName.trim(), remark: remark.trim(), quarter,
    });
  }

  return (
    <Modal title={isEdit ? "Edit self-learning entry" : prefill ? `New course for ${prefill.personName}` : "Add self-learning entry"} onClose={onClose} wide>
      <Field label="Name"><input style={inputStyle} value={personName} onChange={(e) => setPersonName(e.target.value)} placeholder="e.g. Priya Sharma" /></Field>
      <div style={{ display: "flex", gap: 12 }}>
        <div style={{ flex: 1 }}>
          <Field label="Learning type">
            <select style={selectStyle} value={learningType} onChange={(e) => setLearningType(e.target.value)}>
        ...
        <div style={{ flex: 2 }}>
          <Field label={`${learningType} detail`}><input style={inputStyle} value={detail} onChange={(e) => setDetail(e.target.value)} placeholder="e.g. AWS Certified Solutions Architect" /></Field>
        </div>
      </div>
      <div style={{ display: "flex", gap: 12 }}>
        ...
        <div style={{ flex: 1 }}><Field label="% completed"><input type="number" min={0} max={100} style={inputStyle} value={percentCompleted} onChange={(e) => setPercentCompleted(e.target.value)} /></Field></div>
      </div>
      ...
      <div style={{ display: "flex", gap: 12 }}>
        <div style={{ flex: 1 }}>
          <Field label="Assigned by">
            <select style={selectStyle} value={assignedBy} onChange={(e) => setAssignedBy(e.target.value)}>
              <option value="Self">Self</option>
              <option value="Manager">Manager</option>
            </select>
          </Field>
        </div>
        <div style={{ flex: 1 }}><Field label="Manager name (optional)"><input style={inputStyle} value={managerName} onChange={(e) => setManagerName(e.target.value)} placeholder="If assigned by a manager" /></Field></div>
        <div style={{ flex: 1 }}>
          <Field label="Quarter (auto)">
            <div style={{ ...inputStyle, display: "flex", alignItems: "center", color: T.inkMuted, background: T.surface, cursor: "default" }}>
              {endDate ? quarter : "Set an end date"}
            </div>
          </Field>
        </div>
      </div>
      <Field label="Remark (optional)">
        <textarea style={{ ...inputStyle, minHeight: 50, resize: "vertical", fontFamily: T.sans }} value={remark} onChange={(e) => setRemark(e.target.value)} placeholder="Any notes" />
      </Field>
      <Btn style={{ width: "100%", justifyContent: "center", marginTop: 4, opacity: valid ? 1 : 0.5 }} onClick={handleSave}>
...
function EnrollModal({ training, users, onClose, onSave }) {
  ...
  const [selected, setSelected] = useState(training.enrolled.filter((id) => validIds.has(id)));
