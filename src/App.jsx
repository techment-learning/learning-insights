    ...
    <div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12.5, minWidth: 900 }}>
            ...
            <tbody>
              {filteredEntries.map((entry, i) => {
                ...
                return (
                  <tr key={entry.id} style={{ borderBottom: i < filteredEntries.length - 1 ? `1px solid ${T.line}` : "none" }}>
                    <td style={{ padding: "10px 12px", fontWeight: 600, whiteSpace: "nowrap" }}>{entry.personName}</td>
      ...
      {showModal && (
        <SelfLearningEntryModal
          entry={showModal === true ? null : showModal}
          onClose={() => setShowModal(false)}
          onSave={(data) => {
            if (showModal === true) onAdd(data);
...
function SelfLearningEntryModal({ entry, onClose, onSave }) {
  const isEdit = !!entry;
  const selectStyle = { ...inputStyle, cursor: "pointer" };
  ...
  const [assignedBy, setAssignedBy] = useState(entry?.assignedBy || "Self");
  ...
  return (
    <Modal title={isEdit ? "Edit self-learning entry" : "Add self-learning entry"} onClose={onClose} wide>
